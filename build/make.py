...
def make_lib():
  ...
  targets = {
    "arbor.js": ["hermetic.js", "kernel.js", "physics/atoms.js", "physics/system.js", "physics/barnes-hut.js", "physics/physics.js" ]
  }

  for target,deps in targets.items():
    print target

    padding = max(len(os.path.basename(fn)) for fn in deps + (['hermetic.js'] if 'kernel.js' in deps else []))
    ...
    output_code = render_file(target, deps=deps_code)
